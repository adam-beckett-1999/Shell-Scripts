...
DISK_IMAGE="debian-11-generic-amd64.qcow2"
IMAGE_URL="https://cloud.debian.org/images/cloud/bullseye/latest/$DISK_IMAGE"
...
if sudo qm list | grep -qw "5003"; then
    sudo qm destroy 5003
fi

sudo qm create 5003 --name "debian-11-cloudinit-template" --memory 2048 --cores 2 --sockets 2 --cpu cputype=x86-64-v2-AES --net0 virtio,bridge=vmbr0,firewall=0
sudo qm importdisk 5003 "$DISK_IMAGE" storage-01-zfs-iscsi
sudo qm set 5003 --scsihw virtio-scsi-pci --scsi0 storage-01-zfs-iscsi:vm-5003-disk-0
sudo qm set 5003 --boot c --bootdisk scsi0
sudo qm set 5003 --ide2 storage-01-zfs-iscsi:cloudinit
sudo qm set 5003 --serial0 socket --vga serial0
sudo qm set 5003 --onboot 1
sudo qm set 5003 --ostype 126
sudo qm set 5003 --agent enabled=1
sudo qm set 5003 --tags debian-12,cloud-init,template
sudo qm template 5003
