...
if sudo qm list | grep -qw "5001"; then
    sudo qm destroy 5001
fi

sudo qm create 5001 --name "ubuntu-2204-cloudinit-template" --memory 2048 --cores 2 --sockets 2 --cpu cputype=x86-64-v2-AES --net0 virtio,bridge=vmbr0,firewall=0
sudo qm importdisk 5001 "$DISK_IMAGE" storage-01-zfs-iscsi
sudo qm set 5001 --scsihw virtio-scsi-pci --scsi0 storage-01-zfs-iscsi:vm-5001-disk-0
sudo qm set 5001 --boot c --bootdisk scsi0
sudo qm set 5001 --ide2 storage-01-zfs-iscsi:cloudinit
sudo qm set 5001 --serial0 socket --vga serial0
sudo qm set 5001 --onboot 1
sudo qm set 5001 --ostype 126
sudo qm set 5001 --agent enabled=1
sudo qm set 5001 --tags ubuntu-22.04,cloud-init,template
sudo qm template 5001
