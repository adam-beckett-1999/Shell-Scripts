...
# 1. user directory
# 2. vm-id of created template (in this script id 5002 is used)
...
IMAGE_URL="https://cloud-images.ubuntu.com/noble/current/$DISK_IMAGE"
...
if sudo qm list | grep -qw "5002"; then
    sudo qm destroy 5002
fi

sudo qm create 5002 --name "ubuntu-2404-cloudinit-template" --memory 2048 --cores 2 --sockets 2 --cpu cputype=x86-64-v2-AES --net0 virtio,bridge=vmbr0,firewall=0
sudo qm importdisk 5002 "$DISK_IMAGE" storage-01-zfs-iscsi
sudo qm set 5002 --scsihw virtio-scsi-pci --scsi0 storage-01-zfs-iscsi:vm-5001-disk-0
sudo qm set 5002 --boot c --bootdisk scsi0
sudo qm set 5002 --ide2 storage-01-zfs-iscsi:cloudinit
sudo qm set 5002 --serial0 socket --vga serial0
sudo qm set 5002 --onboot 1
sudo qm set 5002 --ostype 126
sudo qm set 5002 --agent enabled=1
sudo qm set 5002 --tags ubuntu-22.04,cloud-init,template
sudo qm template 5002
